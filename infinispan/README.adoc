...
Start Infinispan container in a separate terminal:
    ...
    $ docker run --rm -e USER=admin -e PASS=password -p 11222:11222 -v $PWD/src/test/resources:/user-config/ quay.io/infinispan/server:14.0.28.Final-1 -c /user-config/infinispan.xml
